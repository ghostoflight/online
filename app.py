...
def register_job_in_scheduler(job_id, schedule_str, enabled):
    # BUG FIX #8: safe removal of old job
    ...
    try:
        if schedule_str.startswith("interval:"):
            ...
            if secs < 10:
                # BUG FIX #9: enforce minimum interval to prevent DoS
                secs = 10
            ...
            if len(expr) == 5:
                ...
                print(f"Invalid cron expression for job {job_id}: {schedule_str}")
                return
        else:
            print(f"Unknown schedule format for job {job_id}: {schedule_str}")
            return
...
def update_job(job_id):
    ...
    conn.execute(
        "UPDATE scheduled_jobs SET name=?,code=?,schedule=?,enabled=? WHERE id=?",
